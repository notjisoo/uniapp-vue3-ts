...
// 接受传过来的safeAreaInsets
defineProps<{
  safeAreaInsetsBottom: number | undefined;
}>();
...
const isSelectedAll = computed(() => {
  // every方法，判断数组中的每一项，只有每一项selected是true才是真
  // every方法，如果是空数组也是返回真
...
const onChangeSelectedAll = () => {
  ...
  cartList.value.forEach((item) => {
    return (item.selected = _isSelectedAll);
  });
...
// 计算选中的总金额
const selectedCartListMoeny = computed(() => {
  ...
  <scroll-view @scrolltolower="onScrolltolower" scroll-y class="scroll-view">
    ...
    <template v-if="memberStore.profile">
      ...
      <view
        class="toolbar"
        :style="{
          paddingBottom: safeAreaInsetsBottom ? safeAreaInsetsBottom + 'px' : 0,
        }"
      >
        <text
        ...
        <text class="amount">{{ selectedCartListMoeny }}</text>
    ...
    <view
      ...
      :style="{
        height: safeAreaInsetsBottom ? 200 + 'rpx' : 100 + 'rpx',
      }"
    ></view>
  </scroll-view>
</template>
